...
async function processDocument(client: Pinecone, indexname: string, namespace: string, doc: Document<Record<string, any>>, extractor: FeatureExtractionPipeline) {
  ...
  while(documentChunks.length > 0){
    ...
    await processOneBatch(client,indexname,namespace,extractor,chunkBatch,chunkPatchIndex,filename)
  }
...
async function processOneBatch(client: Pinecone, indexname: string, namespace: string, extractor: FeatureExtractionPipeline, chunkBatch: string[], chunkPatchIndex: number, filename: string) {
